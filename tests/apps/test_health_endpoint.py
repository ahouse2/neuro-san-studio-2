...
def test_health_endpoint_returns_status_keys(monkeypatch):
    ...
    assert {"neo4j", "qdrant", "blocked_requests", "cache"}.issubset(data)
...
def test_health_reports_qdrant_failure(monkeypatch, caplog):
    app = Flask(__name__)
    app.register_blueprint(health_bp)
    client = app.test_client()
    ...
    data = payload["data"]
    meta = payload.get("meta", {})
    assert data["qdrant"] == "fail"
    assert "qdrant_error" in meta and "boom" in meta["qdrant_error"]
