...
Make a copy of `config/.env.sample` to `.env` and fill in the required values. Generate secrets for Flask sessions and JWTs; Neo4j, PostgreSQL and Qdrant run without authentication by default, but you may set passwords if desired:
...
Docker Compose omits database credentials so Neo4j, PostgreSQL and Qdrant start
